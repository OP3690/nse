...
export const HELP = {
  ...
  "sectors.overview": {
    ...
    windowLong: "Today's move plus trailing 1-month, 3-month, 6-month and ~1-year windows.",
    what: "A full read on where money is rotating: each sector's return across four horizons, breadth, smart-money score, drawdown and institutional flow — with a heatmap, rotation map and click-through into constituents.",
    how: [
      "Every liquid stock is grouped by macro sector (sectors with fewer than 3 names are dropped).",
      "Per sector we take the MEDIAN constituent return at each horizon, so one outlier can't distort the read.",
      "Relative strength = the sector's median minus the whole universe's median at that horizon.",
    ],
    benchmarks: [
      { label: "Leading", note: "ahead of market on both 1M & 3M", tone: "up" },
      { label: "Improving", note: "1M turning up, 3M still soft", tone: "accent" },
      { label: "Weakening", note: "3M strong but 1M fading", tone: "amber" },
      { label: "Lagging", note: "behind market on both horizons", tone: "down" },
    ],
    source: "Cross-sectional aggregation of the liquid screener universe + NSE sector indices",
  },
  "sectors.heatmap": {
    title: "Sector Heatmap",
    window: "Selected window (1M–1Y)",
    windowLong: "Colour reflects the median return over whichever window is selected above.",
    what: "Every sector as a tile — coloured green (stronger) to red (weaker) by its median return, sized by aggregate market cap.",
    how: [
      "Tile colour scales with the sector's median return vs the strongest absolute move on screen.",
      "Tile size scales with the sector's total market cap, so the biggest sectors dominate visually.",
      "Click any tile to drill into that sector's full breakdown below.",
    ],
    source: "Median constituent return per sector",
  },
  "sectors.performance": {
    title: "Performance Ranking",
    window: "Selected window (1M–1Y)",
    windowLong: "Sectors ranked by median return over the selected window.",
    what: "Every sector ranked best-to-worst by its median return over the chosen horizon, as a diverging bar.",
    how: [
      "Bars extend right (green) for gains, left (red) for losses, scaled to the strongest move.",
      "Switch the window (1M / 3M / 6M / 1Y) to re-rank instantly.",
    ],
    source: "Median constituent return per sector",
  },
  "sectors.rotation": {
    title: "Rotation Map",
    window: "1M vs 3M, vs market",
    windowLong: "Short-horizon (1M) against long-horizon (3M) relative strength.",
    what: "An RRG-style map: each sector plotted by its 3-month relative strength (x) against its 1-month relative strength (y). The benchmark sits at the origin.",
    how: [
      "X = 3M median return minus the market's 3M median (long-horizon strength).",
      "Y = 1M median return minus the market's 1M median (short-horizon strength).",
      "Top-right Leading, top-left Improving, bottom-right Weakening, bottom-left Lagging.",
    ],
    benchmarks: [
      { label: "Leading", note: "strong & still accelerating", tone: "up" },
      { label: "Improving", note: "recovering — momentum turning up", tone: "accent" },
      { label: "Weakening", note: "still strong but losing momentum", tone: "amber" },
      { label: "Lagging", note: "weak & still decelerating", tone: "down" },
    ],
  ...
  "sectors.detail": {
    ...
    window: "Today · 1M–1Y",
    windowLong: "All horizons for the selected sector, plus its constituents.",
    what: "The selected sector in full: median return at each horizon vs market, breadth, smart-money score, drawdown, turnover, market cap, institutional flow, its leaders and laggards, and every constituent.",
    how: [
      "Each horizon tile shows the sector's median return and its gap to the market median.",
      "Leaders / laggards rank constituents by trailing 1-year return.",
      "The constituents table sorts by the selected window's return; click a symbol for its detail page.",
    ],
    source: "Per-sector aggregation + the liquid screener universe",
  },
  "movers.board": {
